...
export class StudentController {
  ...
  async getAllStudents(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Nếu là nhân viên quản lý, chỉ hiển thị sinh viên trong tòa nhà quản lý
      if (user?.role === 'STAFF' && user?.staffProfile?.managedBuildingId) {
        whereCondition.room = {
          buildingId: user.staffProfile.managedBuildingId
        };
      } else {
        // Nếu không phải nhân viên quản lý, không thêm điều kiện buildingId
        // Điều này cho phép admin xem tất cả các sinh viên
      }

      // Kiểm tra và thêm điều kiện buildingId trước khi tìm kiếm
      if (user?.role === 'STAFF' && user?.staffProfile?.managedBuildingId) {
        whereCondition.room = {
          buildingId: user.staffProfile.managedBuildingId
        };
