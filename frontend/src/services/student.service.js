...
const getStaffById = async (userId) => {
  try {
    ...
    if (response.data?.status === 'success' && response.data?.data) {
      ...
      throw new Error(response.data?.message || `Không tìm thấy nhân viên với User ID ${userId}.`);
    }
  } catch (error) {
    console.error(`Lỗi service getStaffById (${userId}):`, error.response?.data || error.message);
    throw error.response?.data || error;
  }
};
