...
const TransferIndex = () => {
    ...
    const staffBuildingId = user?.profile?.buildingId; // Lấy buildingId từ profile của staff

    // Thiết lập giá trị studentId dựa trên vai trò khi component được mount
    useEffect(() => {
        if (user && isStudent) {
    ...
    const fetchRequests = useCallback(async (page = 1, currentFilters) => {
        ...
        try {
            const params = {
                ...
                buildingId: isStaff ? staffBuildingId : undefined,
            };
