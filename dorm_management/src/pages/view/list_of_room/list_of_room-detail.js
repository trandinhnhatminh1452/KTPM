...
class RoomDetail extends Component {
  ...
  handleSave = () => {
    ...
    console.log("Original room data:", room_list);

    const updatedRoom = {
      ...room_list,
      slot: parseInt(room_list.slot) || 0,
      price: parseFloat(room_list.price) || 0,
      status: Number.isInteger(parseInt(room_list.status))
        ? parseInt(room_list.status)
        : 0,
    };

    console.log("Formatted room data for update:", updatedRoom);
          ...
          <div className="room-detail-item">
            ...
            {isEditing ? (
              <select
                name="status"
                value={
                  room_list.status === undefined ? "" : String(room_list.status)
                }
                onChange={this.handleChange}
              >
