    ...
    async getAllRegistrations(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            // Admin/Staff có thể xem tất cả hoặc lọc theo studentProfileId
                ...
                // Nếu là staff, chỉ cho xem xe của sinh viên trong building mình quản lý
                if (requesterRole === 'STAFF') {
                    ...
                    if (staffProfile?.managedBuildingId) {
                        // Lấy danh sách sinh viên trong building này
                        ...
                        if (students.length > 0) {
                            // Filter theo danh sách studentProfileId của sinh viên trong building
                            options.where!.studentProfileId = {
                                in: students.map(s => s.id)
                            };
                        } else {
                            // Nếu không có sinh viên nào trong building, trả về empty array
                            options.where!.studentProfileId = 0; // 0 không phải là ID hợp lệ
                        }
                    }
                } else if (studentProfileId) {
                    options.where!.studentProfileId = parseInt(studentProfileId as string);
                }
            }
