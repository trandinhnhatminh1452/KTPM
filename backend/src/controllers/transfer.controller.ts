...
export class TransferController {
    ...
    async getAllTransfers(req: Request, res: Response, next: NextFunction) {
        try {
            const { studentProfileId, fromRoomId, toRoomId, status, page, limit, identifier, studentId, buildingId } = req.query;
            ...
            if (userRole === Role.STUDENT) {
                ...
                // Nếu là staff, chỉ cho xem yêu cầu của các sinh viên trong buildingId của staff
                ...
                if (!staffProfile || !staffProfile.managedBuildingId) {
                    return next(new Error('Không tìm thấy tòa nhà được quản lý.'));
                }

                // Lấy danh sách phòng trong buildingId của staff
                const roomsInBuilding = await prisma.room.findMany({
                    where: {
                ...
                if (roomsInBuilding.length > 0) {
                    const roomIds = roomsInBuilding.map(room => room.id);
                    options.where!.OR = [
                        { fromRoomId: { in: roomIds } },
                        { toRoomId: { in: roomIds } }
                    ];
                } else {
                    // Nếu không có phòng nào trong building, trả về mảng rỗng
                    options.where!.id = -1;
                }
            } else {
                // Xây dựng bộ lọc
